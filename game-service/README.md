...
Встроенные шаги сценариев (операции движка), полезные для инвентаря: **`require_inventory_item`** — предмет где угодно в инвентаре (`key_item_def_id`); **`require_item_in_slot`** — предмет в конкретном слоте (`slot`, например `hand_main`, `backpack_0`, и `item_def_id`). См. `pkg/gamekit.InventorySlotHasItem` и `game-service/internal/infrastructure/gameecs/interact_ops.go`.

Выдача лута: **`give_item_to_backpack`** — `item_def_id` в первый свободный слот рюкзака (ошибка при переполнении или если предмет нельзя положить в рюкзак по каталогу; дальнейшие шаги сценария не выполняются). **`spawn_item_on_ground`** — `x`, `y`, `item_def_id`, опционально `layer` (по умолчанию слой выброшенных предметов); ошибка, если клетка на этом слое уже занята. См. `Engine.GiveItemToPlayerBackpack`, `Engine.SpawnPickableItemAt`, `content.TryPutItemInFirstBackpack`.

Удаление триггера: **`remove_interact_tile_at_click`** — без аргументов в JSON (берутся `click_x`, `click_y`, `click_layer` из контекста interact); удаляет тайлы только на этом слое в этой клетке, другие слои не трогает.

Добыча древесины (один шаг): **`wood_harvest`** — аргументы из каталога: `required_slot` (по умолчанию `hand_main`), `required_item_def_id` (`axe`), `reward_item_def_id` (`log`), `ability` (`str` и др. через `gamekit.ParseAbility`), `dc` (по умолчанию 14). Логика: нет нужного предмета в слоте → ошибка, триггер остаётся; провал броска → триггер снимается, лут не выдаётся; успех → выдача в рюкзак, при полном рюкзаке ошибка и триггер остаётся; иначе триггер снимается. Источник случайности для d20 — `Engine.rng` (в `EngineOptions` можно передать `RNG *rand.Rand` для тестов).
...
Сразу после успешного **WebSocket connect** сервер кладёт в исходящую очередь этого соединения **дополнительный** персональный `state` с полным списком **`tiles`** (и актуальными **`players`**), чтобы не ждать следующего глобального full-sync; см. `changelog-server/2026-04-18-initial-full-tiles-on-ws-join.md` и `docs/tick_invariants.md`.
...
| `PlayerSprite` | `Name` — id листа ходьбы (`state.sprite` / `character.data.sprite`); старый снимок без компонента → **`DefaultPlayerSprite`** при миграции |
