...
type DamageSystem struct {
	mapper *ecs.Map7[gamekit.PlayerRef, gamekit.GridPos, gamekit.Speed, gamekit.Health, gamekit.PlayerFace, gamekit.CharacterStats, gamekit.PlayerSprite]
}

// NewDamageSystem создаёт систему урона с общим mapper мира.
func NewDamageSystem(mapper *ecs.Map7[gamekit.PlayerRef, gamekit.GridPos, gamekit.Speed, gamekit.Health, gamekit.PlayerFace, gamekit.CharacterStats, gamekit.PlayerSprite]) *DamageSystem {
...
func (s *DamageSystem) applyDamage(targetEnt ecs.Entity, damage int) {
	...
	_, _, _, hp, _, _, _ := s.mapper.Get(targetEnt)
	hp.HP -= damage
	if hp.HP < 0 {
		hp.HP = 0
	}
